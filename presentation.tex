...
\begin{frame}[t]{LLM-as-a-Judge Prompt}
\raggedright
\small
...
\begin{frame}[t]{Metric: LLM-as-a-judge\\Model: o3}
\centering
\raggedright
...
\raggedright
\begin{itemize}
    \item \textbf{LSTM = 0.25} (gpt-4o-mini): explanation is often generic and weakly grounded in concrete code changes.
    \item \textbf{Adam = 0.85} (gpt-4.1): explanation is algorithm-specific, technically precise, and aligned with the produced code.
    \item Main signal: quality differences are most visible on optimization-heavy algorithms.
    \item Next focus: prompt tuning + targeted regression checks on low-scoring tasks (LSTM, BatchNorm).
\end{itemize}
\end{frame}
