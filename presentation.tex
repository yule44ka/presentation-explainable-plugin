...
\begin{frame}[t]{SWE-bench Verified Mini: Django subset}
\scriptsize
\setlength{\leftmargini}{1.2em}
\textbf{What is in the dataset}
\begin{itemize}\setlength{\itemsep}{0.08cm}
    \item Real bug-fix tasks from \texttt{django/django}.
    \item For each task: GitHub issue text, starting commit, Django version, and setup commit.
...
\begin{itemize}\setlength{\itemsep}{0.08cm}
    ...
    \item Keeps evaluation focused on one codebase, so results are easier to compare and explain.
...
\begin{frame}{Results}
\centering
\includegraphics[width=0.72\linewidth,height=0.68\textheight,keepaspectratio]{django_success_count_comparison.png}

\vspace{0.12cm}
\footnotesize Solved Django tasks before and after adding explanations.
\end{frame}

\begin{frame}{Results}
\centering
\includegraphics[width=0.96\linewidth,height=0.68\textheight,keepaspectratio]{django_agent_actions_with_passed_tests.png}

\vspace{0.12cm}
\footnotesize Agent action counts on Django tasks with passed evaluation.
\end{frame}
